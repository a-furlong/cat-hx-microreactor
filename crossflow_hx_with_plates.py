    ...
    def solvehx(self):
        ...
        for j in range(0, self.dims[3]):
            for i in range(0, self.dims[2]):
                ...
                self.Q_utilityFluid[i, j] = self.utility_h[i, j]*self.hx_area_uPuF*(self.T_utilityPlate[i, j] - self.utility_T[i, j])
                self.Q_utilityPlate[i, j] = -self.Q_utilityFluid[i, j] + self.metalk*self.hx_area_uPrP*(self.T_reactantPlate[i, j] - self.T_utilityPlate[i, j])/self.dims[5] + self.reactant_h[i, j]*self.hx_area_uPrF*(self.reactant_T[i, j] - self.T_utilityPlate[i, j])
                self.Q_reactantsFluid[i, j] = -self.reactant_h[i, j]*self.hx_area_uPrF*(self.reactant_T[i, j] - self.T_utilityPlate[i, j]) - self.reactant_h[i, j]*self.hx_area_rPrF*(self.reactant_T[i, j] - self.T_reactantPlate[i, j])
                self.Q_reactantsPlate[i, j] = self.reactant_h[i, j]*self.hx_area_rPrF*(self.reactant_T[i, j] - self.T_reactantPlate[i, j]) - self.fuel_h[i, j]*self.hx_area_rPfF*(self.T_reactantPlate[i, j] - self.fuel_T[i, j]) - self.metalk*self.hx_area_rPfP*(self.T_reactantPlate[i, j] - self.T_fuelPlate[i, j])/self.dims[5] - self.metalk*self.hx_area_uPrP*(self.T_reactantPlate[i, j] - self.T_utilityPlate[i, j])/self.dims[5]
                self.Q_fuelFluid[i, j] = self.fuel_h[i, j]*self.hx_area_rPfF*(self.T_reactantPlate[i, j] - self.fuel_T[i, j]) - self.fuel_h[i, j]*self.hx_area_fPfF*(self.fuel_T[i, j] - self.T_fuelPlate[i, j])
                self.Q_fuelPlate[i, j] = self.fuel_h[i, j]*self.hx_area_fPfF*(self.fuel_T[i, j] - self.T_fuelPlate[i, j]) + self.metalk*self.hx_area_rPfP*(self.T_reactantPlate[i, j] - self.T_fuelPlate[i, j])/self.dims[5]
                ...
                self.utility_T[i+1, j] = self.utility_T[i, j] + self.Q_utilityFluid[i, j]/(self.reactant_in[1]/self.dims[2]*self.reactant_cp[i, j])
                ...
                self.reactant_rho[i, j+1], self.reactant_mu[i, j+1], self.reactant_cp[i, j+1],self.reactant_k[i, j+1], self.reactant_Pr[i, j+1] = self.fluid_properties_fuel(self.reactant_T[i, j+1], self.reactant_P[i, j+1])
        ...
        for n in range(1):
            for j in range(1, self.dims[3]):
                    for i in range(1, self.dims[2]):
                        ...
                        #use the mass flow rate through each channel instead of the velocity in each channel for the heat transferred
                        self.utility_T[i, j] = self.utility_T[i, j] + self.Q_utilityFluid[i, j]/(self.reactant_in[1]/self.dims[2]*self.reactant_cp[i, j])
                        ...
                        #update thermophysical and transport properties for new conditions
                        self.reactant_rho[i, j], self.reactant_mu[i, j], self.reactant_cp[i, j],self.reactant_k[i, j], self.reactant_Pr[i, j] = self.fluid_properties_fuel(self.reactant_T[i, j], self.reactant_P[i, j])
                        ...
                        #update velocity
                        self.reactant_u[i, j] = self.reactant_u[i, j]*self.reactant_rho[i, j]/self.reactant_rho[i, j]
                        self.utility_u[i, j] = self.utility_u[i, j]*self.utility_rho[i, j]/self.utility_rho[i, j]
                        self.fuel_u[i, j] = self.fuel_u[i, j]*self.fuel_rho[i, j]/self.fuel_rho[i, j]
...
reactantin = [{'O2':5, 'CO2':92, 'H2O':3, 'CH4':0}, 0.001, 1000, 1500000]
utilityin = [{'H2O': 100}, 0.0005, 650, 1000000]
fuelin = [{'CH4': 100}, 0.0005, 405, 1500000]
dimensions = [0.005, 0.005, 15, 20, 0.0002, 0.01]
...
sol = solve_ivp(hx1.transientHX, [0, 0.10], initialTemps, method = "BDF", t_eval = [0, 0.05, 0.10])

#grab final temperature profiles and reshape into given form
n = 2 #take column 2
reactant_T_out = sol['y'][0:(dimensions[2]*(dimensions[3]))]
reactant_T_out = reactant_T_out[:, 2]
...
reactantPlate_T_out = reactantPlate_T_out[:, 2]
...
utilityPlate_T_out = utilityPlate_T_out[:, 2]
...
fuelPlate_T_out = fuelPlate_T_out[:, 2]
